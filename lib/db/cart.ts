...
export async function addToCart(userId: string, item: Omit<CartItem, "productId"> & { productId: string }) {
  try {
    const client = await clientPromise
    const db = client.db("bucci")
...
export async function updateCartItem(userId: string, productId: string, size: string, color: string, quantity: number) {
  try {
    const client = await clientPromise
    const db = client.db("bucci")
    ...
    console.error(`Error updating cart item for user with ID ${userId}:`, error)
    throw error
  }
}

export async function removeFromCart(userId: string, productId: string, size: string, color: string) {
  try {
    const client = await clientPromise
    const db = client.db("bucci")
